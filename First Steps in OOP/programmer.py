...
# o	In the last case, the programmer does not have enough skills, so
...
class Programmer:
    ...
    def change_language(self, new_language, skills_needed):
        if self.skills >= skills_needed and new_language != self.language:
            before_change = self.language
            self.language = new_language
            return f"{self.name} switched from {before_change} to {new_language}"
